function KernelKmeans_equal_weight_multi_kernel(dataset, kernel_type, nRepeat)
...
ew_kkm_res_file = fullfile(res_dir, [dataset, '_res_kkm_ew.mat']);
if ~exist(ew_kkm_res_file, 'file')
    t_start = clock;
    ew_kkm_res = KernelKmeans_single_kernel(Ka, y, fullfile(res_dir, [dataset, '_ewmk']), nRepeat);
    t_end = clock;
    disp(['KernelKmeans exe time: ', num2str(etime(t_end, t_start))]);
    save(ew_kkm_res_file, 'ew_kkm_res');
else
    load(ew_kkm_res_file, 'ew_kkm_res');
end
disp(['KernelKmeans on on equal weighted multi kernel done']);
if size(ew_kkm_res, 1) > 1
	res_ew_kkm_aio = mean(ew_kkm_res);
else
    res_ew_kkm_aio = ew_kkm_res;
end
clear K ew_kkm_res;

save(fullfile(res_dir, [dataset, '_res_kkm_ew_kernel.mat']), 'res_ew_kkm_aio', 'kernel_list');
end