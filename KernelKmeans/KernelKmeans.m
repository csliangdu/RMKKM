function [label, center, bCon, sumD, D] = KernelKmeans(K, k, varargin)
%   Kernel K-means clustering, accelerated by matlab matrix operations.
%
...
for t=1:reps
    ...
    while any(label ~= last) && it<maxit
        ...
        center = full(sparse(1:n,label,1,n,k,n)); % indicator matrix
        center = bsxfun(@rdivide, center, max(sum(center), 1e-10)); % weighted indicator
        it=it+1;
    end
    if it<maxit
